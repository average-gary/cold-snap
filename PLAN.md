...
the linked image is **379,940 B = 26.6548%** from `cargo build --release`
(**this read 379,648 B = 26.6343% from 2026-09-12 until 2026-09-17 and 380,040 B = 26.6618% until 2026-09-18**; the net +292 B is `hal/src/psram.rs` gaining its first production caller plus the new `firmware/src/upgrade.rs` — UPGRADE-PLAN §7b/§7d)
...
field-upgradable**. Installation uses Coldcard's existing key-zero DFU path,
inheriting a permanent warning screen and forced delay on every boot. That is
unavoidable and by design (`docs/dev-access.md`).

**This section read "Frostsnap's `secure_boot.rs` and `ota.rs` have no role" until
2026-09-17, and half of that is now false.** `ota.rs` is the PORTED SHAPE for
`firmware/src/upgrade.rs`: its standalone-mode-with-its-own-state-enum structure, its
`byte_count == upgrade_size` termination, the positional 4,096-byte chunk boundary and
the `0x11` discipline all came from it, and phase 3's own doc comments pin it
throughout. What did NOT port is everything ESP32-specific — the A/B slot machinery,
`switch_partition`, otadata CRC32, sector erase, the baud change. `secure_boot.rs`
genuinely still has no role: the Mk4 bootloader verifies the staged image inside the
firewall ~100 ms later and only its verdict counts, so the device keeps a cheap sha256
digest check and no signature check at all. See UPGRADE-PLAN §7b.

The "never field-upgradable" claim is about the BOOTLOADER and remains exactly true.
What phase 3 added is a path for upgrading the APPLICATION image the bootloader
verifies, which is a different thing and needs that bootloader intact to work.
...
(`firmware/`, which links at 379,940 B under `cargo build --release` — **this read 379,648 B until 2026-09-17 and 380,040 B until 2026-09-18**), `encode_frame` has a production caller
...
4. **Whether the flash driver must execute from RAM is unresolved**, and the old
   ...
   **8 KiB `DBANK0_PAGE`**, not of the 4 KiB `ERASE_SIZE`, and that is the whole
   ...
   existing file rather than new work; and the margin is not 44 K but **1,045,468 B** —
   the linked image is 379,940 B of a 1,425,408-byte `FLASH_TEXT` (`cargo build
   --release`; **this read 1,045,760 B against 379,648 B until 2026-09-17 and 1,045,368 B against 380,040 B until 2026-09-18**), and the margin is that subtraction so the two cannot drift apart again.
...
5. ~~**PSRAM capacity is unverified from Coldcard source.**~~ **CLOSED as declined,
   ...
   short of space. **This item's last sentence read "Declined rather than deferred — this
   firmware does not use PSRAM, and adopting it would mean inheriting the OSPI/QUADSPI
   state the bootloader leaves configured, for no measured benefit" until 2026-09-17.**
   It is now false in its premise and its conclusion. The firmware DOES use PSRAM:
   `hal/src/psram.rs` gained its first production caller in `firmware/src/upgrade.rs`,
   which stages a firmware image in the lower half of it (UPGRADE-PLAN §7b). The
   "measured benefit" that did not exist then is the only reflash path this hardware can
   have — at RDP=2 there is no DFU and no SWD, so without staging every iteration costs a
   device. The OSPI/QUADSPI worry was also overstated: `psram_setup()` runs on the
   straight-line boot path BEFORE the `verify_firmware()` that gates the jump into our
   image, so there is nothing to inherit and nothing to initialise. What is still true is
   the heap sentence above it: PSRAM changes no allocation decision, because staging is
...
### 8.2b Upgrade phase-3 register — 40 mutations run, 35 caught, 5 survived-green, 2 real defects

**Landed 2026-09-17.** Numbering note: this is UPGRADE-PLAN.md's phase 3 (firmware-upgrade
staging), not §8.2's, which is this file's own original phase 3. Two different phasings, one
tree; the register lives here because that is where registers live, and the FULL table lives
in **UPGRADE-PLAN.md §7b/§7c/§7d** because that file owns the measured gate table. Duplicating 35
rows here would give every one of them a second home to rot in — which is the drift §9's
claim-hygiene item exists to stop — so this section carries only what a reader of THIS file
needs.

**This heading read "22 mutations, 4 survived-green" and this paragraph read "18 device-side
mutations plus 4 leg-level ones … 21 caught" until 2026-09-18.** Those figures were wrong three
ways at once — 18 + 4 = 22 total cannot coexist with 21 caught + 4 survived = 25, the table they
describe has 19 device-side rows, and UPGRADE-PLAN §7b:392-400 had already retracted them. The
authoritative accounting, and it is a SUM of three dated passes, not one number: **§7b** 27 run /
23 caught (19 device-side + 4 leg-level) / 4 survived-green; **§7c** (the hardening pass, an
independent re-run of all 27 plus 8 new) 8 run / 7 caught / 1 survived-green; **§7d** (the fix
pass) 5 run / 5 caught / 0 survived. **40 run, 35 caught, 5 survived-green.** Every one RUN, and
every file restored byte-identical (sha256 before == after, checked per run).

**The five survived-green / withdrawn results, which are the reason this section is not just
a link:**

1. **A state reset in `Stager::admit` is DEAD.** Deleting `self.state = State::Idle;` left
   all 191 firmware tests GREEN, exit 0 — every exit from that arm assigns the state anyway,
   so the invariant holds by exhaustion over four paths rather than unconditionally. Kept and
   labelled; the test's doc was corrected to name the `written = 0` mutation it does catch.
2. **A 100 ms transport gap that the harness cannot test.** M13 asserted that deleting it
   reddened the positive leg. Deleting it left every leg GREEN: a pty delivers separate
   writes as separate reads, so the race it guards never materialises off USB. Kept for
   upstream parity and labelled unproven at the call site.
3. **A DOMINATED leg was deleted rather than shipped**, in the §3069 tradition: a fourth M13
   leg forced a lost chunk head and asserted no ack arrived (correct, MEASURED 0 acks), but
   every device mutation that could make it fire was caught by the negative leg first.
   Replaced by one that is uniquely falsifiable.
4. **A green M13 hid an intermittent bug in its own handshake.** A leftover `MAGIC_REPLY`
   (first byte `0x00`) was read as a chunk ack, failing ~1 run in 3. Nothing in the ordinary
   event loop can see it — only a leg that switches to raw byte reads.
5. **A bound that is UNFALSIFIABLE rather than untested** (§7c). Deleting `check_read` from
   `FakePsram::view` left all 316 hal tests GREEN, and it cannot do otherwise: `FakePsram::new`
   panics above `PSRAM_STAGE_LEN`, so the capacity refuses every `len` the window bound would,
   and both answer the same `OutOfBounds`. The test's doc CLAIMED that leg pinned the ordering;
   the claim was false and is rewritten. `MappedPsram::view`'s `check_read` — the one that
   matters — IS covered by mutation.

**Two REAL production defects the mutation work found**, both invisible to review:

* `upgrade::run` recorded the admitted message in a single `Option`, so two `Upgrade` frames
  in ONE 64-byte read left only the second admitted — `EnterUpgradeMode` from `Idle`, which
  is a refusal and an upgrade that could never start. This is §8.1's defect-5 class again:
  the harness's transport happened not to produce the coalescing.
* `Stager::feed` zeroed a part-filled word carry, so feeding one byte at a time dropped three
  of every four bytes. Caught only because that test streams at 1, 2 and 3 rather than at the
  realistic 64.

**The pattern worth carrying forward:** four of the five survived-green results are
*harness* facts, not device facts — the pty, the leg ordering, the framer, and the double's
own capacity. Only item 1, the dead state reset, is a fact about device code. (**This read
"three of the four … the pty, the leg ordering, the framer" until 2026-09-18**, a count and an
enumeration left behind when §7c added item 5 six lines above it and README.md:192 was corrected
in the same pass this sentence was not — the parallel-copy miss this very section exists to
record.) §8.1's lesson was
that a green suite proves nothing about the DEVICE; this register's lesson is that a green
suite also proves nothing about the HARNESS, and a claim about a transport race needs a
transport that can produce it.
...
14. **CLOSED 2026-08-25 — the identity hold now says why.** `ui::identity_fault`
    ...
    loop. **This sentence read "The hold stays above USB bring-up, so the security
    property is unchanged" until 2026-09-17** and is WITHDRAWN: phase 3 moved USB to boot
    step 6c, ~190 lines above the hold (UPGRADE-PLAN §3.7, DECISIONS.md decision 8(d)).
    The security property survives for a DIFFERENT reason — the hold never polls `cdc`, so
    enumeration never COMPLETES, no handshake is answered and no frame is decoded; what a
    host sees is one attach event. The archived original below therefore also carries a
    withdrawn prohibition: "the fix is **not** to bring USB up early to obtain a
    diagnostic" is precisely what phase 3 did, deliberately and with its cost stated,
    because at RDP=2 a device with a damaged identity record otherwise has no reflash path
    at all. Pinned by `identity_fault_names_the_state_and_refuses_rather_than_inviting_a_retry`.
...
22. **184 `cfg(target_arch = "arm")` blocks were invisible to every LINT in this project.
    ...
    **Three more corrected 2026-09-17**, all found in passing while implementing
    UPGRADE-PLAN §7b rather than by a sweep — the cadence finding below in miniature.
    (a) `firmware/src/main.rs` cited `usb::CdcPort::poll`; **there is no `CdcPort` anywhere
    in `hal/src` or `firmware/src`** — the type is `usb::Cdc` — so this was not drift but a
    name that never existed, and the only occurrence of the string in the tree was that one
    comment. (b) `firmware/examples/checkfw.rs` put `pub fn firmware_digest` at
    `lib.rs:2956`; it is at `:3003`, 47 lines stale — and that is the same citation which
    already carries a "this said `:615` until 2026-09-12" clause, so it is the running
    demonstration of its own argument for citing by SYMBOL. (c) `hal/src/psram.rs` carried an
    `rg` recipe asserting the module had **no production caller**, and an "ARM image unmoved
    at 379,648 B" resting on it; both went false the moment `firmware/src/upgrade.rs` landed,
    and the module doc now says so with the date. All three are source-side, which is where
    1,158 of the 1,449 citations live.

    **And (c) REPRODUCED THE CADENCE FINDING BELOW WITHIN ONE ROUND, which is why it is
    written up rather than quietly extended.** Fixing the `rg`-recipe paragraph in
    `hal/src/psram.rs` left FOUR live siblings asserting the opposite, one of them the very
    line the rewrite had named as the recipe's result set: `psram.rs`'s module-doc "accessor
    with no production caller", `MappedPsram`'s own "**No production caller.**",
    `hal/src/lib.rs`'s crate-root module table "**No production caller, by design**", and
    `memmap::PSRAM_BASE`'s "**Read and written by no production code** … it has no caller" —
    and `hal/src/lib.rs` was not in that change's diff at all. **Corrected 2026-09-18**
    (UPGRADE-PLAN §7d item 3), each with the dated clause. Same round, same shape, three more:
    the identity-fault "the hold sits above USB bring-up" claim had FOUR homes and one was
    rewritten (`hal/src/identity.rs`, `hal/src/ui.rs` and §9 item 14 above were the survivors,
    with `identity.rs` pointing the reader at the `main.rs` site that now contradicts it —
    §7d item 4); §10's image figure stayed at 379,648 B while README, DECISIONS.md's Provenance
    row and UPGRADE-PLAN §7b all moved, so **the authority disagreed with all three documents
    that name it as the authority** (§7d item 5); and this file's §8.2b printed a mutation tally
    UPGRADE-PLAN §7b had already retracted (§7d item 7). **The lesson is a procedure, not a
    resolution: when a claim is found false, grep the CLAIM's distinctive phrase tree-wide
    before editing the site that surfaced it.** `rg -F 'no production caller'` and
    `rg -F 'above USB bring-up'` each take one second and each would have found every home.
