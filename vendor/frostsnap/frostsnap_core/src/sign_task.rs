...
impl CheckedSignTask {
    ...
    /// How many signatures this task requires, i.e. `sign_items().len()`, without
    /// doing the taproot sighash work `sign_items` does. Used to validate the
    /// coordinator's `agg_nonces` length before signing.
    pub fn n_sign_items(&self) -> usize {
        match &self.inner {
            SignTask::Test { .. } | SignTask::Nostr { .. } => 1,
            SignTask::BitcoinTransaction { tx_template, .. } => {
                tx_template.iter_locally_owned_inputs().count()
            }
        }
    }
...
pub enum SignTaskError {
    ...
    NothingToSign,
    /// The coordinator sent a number of aggregated nonces that does not match the
    /// number of signatures the task requires.
    WrongNumberOfNonces {
        got: usize,
        expected: usize,
    },
...
impl core::fmt::Display for SignTaskError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            SignTaskError::WrongNumberOfNonces { got, expected } => {
                write!(
                    f,
                    "Coordinator sent {got} aggregated nonces but the task needs {expected}"
                )
            }
        }
    }
