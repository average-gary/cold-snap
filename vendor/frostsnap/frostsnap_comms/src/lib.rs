...
///
/// LOCAL MODIFICATION (cold-snap), 2026-08-19: was `1 << 15` = 32,768. Lowered to
/// 20,480 = `coldsnap_hal::comms::ENCAPS_DECODE_LIMIT`, so ONE number bounds both
/// encapsulated legs. Cleared by measurement: the largest well-formed frame the
/// transport admits at all (`FRAME_LIMIT` = 4,096; 61 nonces in one segment, frame
/// 4,086 B) needs **17,664 B** on the device->coordinator leg in debug, 15,360 in
/// release, and the coordinator->device leg's worst is ~17,702. Effective budget is
/// 20,472 (bincode's `Limit<L>` charges the 8-byte length claim against the same
/// budget), so this refuses nothing a frame-bounded message can carry in either
/// direction in either profile. Sized off the DEBUG column deliberately: a bound
/// whose position moves with `debug_assertions` is PLAN.md §8.1's divergence class,
/// and 8,192 has exactly that defect here (refuses a real 30-nonce `NonceResponse`
/// in debug at 8,704, admits it in release at 7,424). Decode-side only — bincode's
/// encoder never reads the limit, so zero wire bytes change.
/// Derivation: `tools/research-scratch/device_send_alloc_measure.rs`.
const MAX_MESSAGE_ALLOC_SIZE: usize = 20_480;
...
impl EncapsBody {
    /// COLD-SNAP LOCAL ADDITION (see cold-snap `vendor/README.md`). Additive only:
    /// no wire change, no behaviour change, nothing upstream reads it.
    ///
    /// It exists so the nested decode can be bounded by the *device*.
    /// `WireCoordinatorSendBody::decode` re-enters bincode here under the crate-wide
    /// `BINCODE_CONFIG`, which is far above anything legitimate and is remotely
    /// reachable; `coldsnap_hal::comms::decode_body` decodes these bytes under its
    /// own tighter limit instead. The field is otherwise private with no accessor,
    /// so there is no way to do that without this.
    ///
    /// Still required after `MAX_MESSAGE_ALLOC_SIZE` came down to the same 20,480:
    /// keeping the refusal in *our* code is what stops the bound silently reverting
