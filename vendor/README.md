...
**The scope sentence above named `device.rs` while the table had NO `device.rs` row and no
`tweak.rs` row, from the day it was written until 2026-09-12.** Three live sites were
missing, all found while implementing screen 8; they are the second table below. The
distinction the two tables draw is the one that matters here: **the rows above are FIXED IN
THE VENDORED TREE** (which is why they must be re-applied on every re-vendor, and why the
rebase recipe says so), while **the rows below are ROUTED AROUND IN OUR OWN CODE and are
still panics in `vendor/`**. Confusing the two is how a "simplification" of our dispatch
re-arms one of them.
...
| `frostsnap_embedded/src/ab_write.rs` | `AbSlot::new` given a `# Panics` doc (`:17`) | class (a), priority 3: comment only. **One citation INSIDE that added doc is stale and is deliberately left in the vendored file, corrected here instead (2026-09-12):** it says the partition layout is fixed by the caller "(`nonce_slots.rs:14-16` only ever splits off exactly 2 sectors)", but `:14-16` is the `last_write` field's doc. The caller is `NonceAbSlot::load_slots` (`nonce_slots.rs:23-32`), whose `AbSlot::new(partition.split_off_front(2))` is at `:27`. Recorded here rather than patched so this row stays a comment-only change with no new rebase surface. |
...
#### Panic sites NOT fixed here — routed around in our code, still live in `vendor/`

Added 2026-09-12. `../PLAN.md` §8.3 carries the same three with their triggers and
reproductions. Per PUNT EVERYTHING UPSTREAM (`../DECISIONS.md` decision 6) nothing is
filed and nothing here is patched: our own code takes a different path, and a re-vendor
therefore needs **no** action on these rows — only a re-read, because if our route ever
changes the panic is back. Cited by SYMBOL: `device.rs` is byte-identical to upstream
`d8b0525`, and `tweak.rs` differs only by decision 3, whose insertion SHIFTS the line
number of the row-3 site.

| Site (by symbol) | Panic | What routes around it |
|---|---|---|
| `frostsnap_core/src/device.rs`, `FrostSigner::recv_coordinator_message`'s `ScreenVerify(ScreenVerify::VerifyAddress { .. })` arm | `self.wallet_network(key_id).expect("cannot verify address on key that doesn't support bitcoin")` — **REACHABLE ON ONE WIRE FRAME** with any key whose purpose is `KeyPurpose::Test` or `Nostr`, since the arm's own key-existence check has already returned a clean error for an unknown key. REPRODUCED as a panic at `device.rs:411:22` | `coldsnap_firmware::Session::recv_core`'s own `ScreenVerify` arm destructures and **RETURNS** before the fall-through to `signer.recv_coordinator_message`; `Session::verify_prompt` spells the same lookup `.ok_or(Refusal::AddressVerify)?`. Pinned: replacing that arm with `ScreenVerify(_) => {}` makes a named test PANIC |
| `frostsnap_core/src/device.rs`, same arm, next line | `bitcoin::Address::from_script(&spk.spk(), network).expect("has address form")` | the same `return`. `Session::verify_prompt` spells it `.map_err(\|_\| Refusal::Undisplayable)?`. LIVE but shadowed — the row above fires first on the only route in, so it is not independently reproduced |
| `frostsnap_core/src/tweak.rs`, `<DerivationPath as DerivationPathExt>::from_normal_path_segments`, reached from `impl From<BitcoinBip32Path> for DerivationPath` | `ChildNumber::from_normal_idx(..).expect("valid normal derivation index")` errs for every index with bit 31 set, and `BitcoinBip32Path.index` is a raw `pub u32` that is `bincode::Decode`. REPRODUCED as `InvalidChildNumber(4294967295)` | **NO in-tree caller.** `coldsnap_firmware` composes a `ui::Buf::<16>` of `"Recv #"` plus the decimal index rather than formatting a `DerivationPath`, precisely to avoid this. Pinned: the naive `DerivationPath::from(*bip32_path).to_string()` mutation panics in the `u32::MAX` test — the index-0 test passes straight through it |
...
unmaintained ledger is what made it two images stale twice: **this clause read
`firmware/` links at **376,752 B = 26.43%** of `FLASH_TEXT`" until 2026-09-12**, by which
point §10 said 379,648 B = 26.6343%. LTO plus `--gc-sections` keeps only what is
reachable. Read the table for what it is good for —
...
reachable will be [the answer]" — is now paid: see `../PLAN.md` §10 for the figure (**this sentence ended "it is
376,752 B" until 2026-09-12, two images stale**; the quoted promise above it is withdrawn
text and is left verbatim on purpose).
